...
export const ManageSubscriptionContent: React.FC<ManageSubscriptionContentProps> = ({ subscription, view, onClose }) => {
    ...
    // Upgrade State
    const [selectedPlanToUpgradeId, setSelectedPlanToUpgradeId] = useState<string>('');
    ...
    useEffect(() => {
        if (activeTab === 'upgrade') {
            if (subscription.category === 'cloudedge') {
                ...
                const isBundle = subscription.bundledPlans && subscription.bundledPlans.length > 0;
                
                if (isBundle) {
                    const firstPlan = subscription.bundledPlans![0];
                    setSelectedPlanToUpgradeId(firstPlan.planId);
                    const currentPlanObj = PLANS.find(p => p.id === firstPlan.planId);
                    const nextPlan = PLANS.find(p => p.priceMonthly > (currentPlanObj?.priceMonthly || 0));
    ...
    // Update selectedUpgradePlanId when selectedPlanToUpgradeId changes for bundles
    useEffect(() => {
        if (activeTab === 'upgrade' && subscription.category === 'posta' && subscription.bundledPlans && subscription.bundledPlans.length > 0) {
            const planToUpgrade = subscription.bundledPlans.find(p => p.planId === selectedPlanToUpgradeId);
            if (planToUpgrade) {
                const currentPlanObj = PLANS.find(p => p.id === planToUpgrade.planId);
                const nextPlan = PLANS.find(p => p.priceMonthly > (currentPlanObj?.priceMonthly || 0));
                setSelectedUpgradePlanId(nextPlan?.id || PLANS[0].id);
            }
        }
    }, [selectedPlanToUpgradeId, activeTab, subscription]);
    ...
    const renderOverview = () => {
        ...
        const postaSpecs = [
             ...
             { label: 'Active Licenses', value: isBundle ? `${subscription.bundledPlans?.reduce((acc, p) => acc + p.quantity, 0)} Users` : '10 Users' },
            ...
            <div className="space-y-6 animate-fade-in">
                {isBundle && (
                    <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 p-4 rounded-xl flex items-start gap-3">
                        <Icon name="fas fa-layer-group" className="text-amber-600 mt-1" />
                        <div>
                            <p className="font-bold text-amber-900 dark:text-amber-200 text-sm">Bundled Subscription</p>
                            <p className="text-xs text-amber-800 dark:text-amber-300">This subscription contains multiple plans managed as a single billing unit.</p>
                        </div>
                    </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {isBundle && (
                    <Card title="Bundled Plans Details">
    ...
    const renderRenew = () => {
        ...
        let isBundle = subscription.bundledPlans && subscription.bundledPlans.length > 0;
        let basePrice = 0;
        let yearlyPrice = 0;

        if (isBundle) {
            subscription.bundledPlans?.forEach(p => {
                const planDetails = PLANS.find(pl => pl.id === p.planId);
                if (planDetails) {
                    let monthly = planDetails.priceMonthly;
                    if (p.advancedRulesEnabled && !['light', 'business'].includes(p.planId)) {
                        monthly += 2.00;
                    }
                    basePrice += monthly * p.quantity;
                    yearlyPrice += (monthly * 12 * p.quantity) * 0.83;
                }
            });
        } else {
            ...
            basePrice = currentPlan ? currentPlan.priceMonthly : 100;
            yearlyPrice = currentPlan ? (currentPlan.priceYearly || basePrice * 12 * 0.83) : basePrice * 12;
        }
        ...
        return (
            <div className="space-y-6 animate-fade-in">
                {isBundle && (
                    <div className="bg-blue-50 dark:bg-sky-900/10 border border-blue-200 dark:border-sky-800 p-4 rounded-xl flex items-start gap-3">
                        <Icon name="fas fa-info-circle" className="text-blue-600 mt-1" />
                        <div>
                            <p className="font-bold text-blue-900 dark:text-blue-200 text-sm">Consolidated Renewal</p>
                            <p className="text-xs text-blue-800 dark:text-blue-300">Renewing this subscription will extend all {subscription.bundledPlans?.length} bundled plans for the same period.</p>
                        </div>
                    </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    const renderUpgrade = () => {
        ...
        if (isPosta) {
            ...
            if (isBundle) {
                const planToUpgrade = subscription.bundledPlans?.find(p => p.planId === selectedPlanToUpgradeId);
                const currentPlanObj = PLANS.find(p => p.id === selectedPlanToUpgradeId);
                currentPlanPrice = currentPlanObj ? currentPlanObj.priceMonthly : 0;
                currentPlanName = currentPlanObj ? currentPlanObj.name : '';
            } else {
        ...
        return (
            <div className="space-y-8 animate-fade-in">
                ...
                {/* 0. Select Plan to Upgrade (Only for Bundles) */}
                {isBundle && (
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {subscription.bundledPlans?.map(p => {
                                const isSelected = p.planId === selectedPlanToUpgradeId;
                                return (
                                    <div key={p.planId} onClick={() => setSelectedPlanToUpgradeId(p.planId)} className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex flex-col ${isSelected ? 'border-[#679a41] bg-green-50 shadow-md ring-1 ring-[#679a41]/50' : 'border-gray-200 hover:border-gray-300'}`}>
                                        <div className="flex justify-between items-start mb-1">
                                            <h4 className="font-bold text-[#293c51] dark:text-gray-100">{p.planName}</h4>
                ...
                <div>
                    <h3 className="text-lg font-semibold mb-4 text-[#293c51] dark:text-gray-100 flex items-center gap-2">
                        <span className="w-6 h-6 rounded-full bg-[#679a41] text-white flex items-center justify-center text-xs">{isBundle ? '2' : '1'}</span>
                        Choose New Plan
                    </h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-[#293c51] dark:text-gray-100 flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-[#679a41] text-white flex items-center justify-center text-xs">{isBundle ? '3' : '2'}</span>
                    ...
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-[#293c51] dark:text-gray-100 flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-[#679a41] text-white flex items-center justify-center text-xs">{isBundle ? '4' : '3'}</span>
...
export const ManageSubscriptionPage: React.FC = () => {
    ...
    const subscription = useMemo(() => {
        const mockSub = mockSubscriptions.find(s => s.id === subscriptionId);
        if (mockSub) return mockSub;
        
        const newSubs = JSON.parse(localStorage.getItem('new_subscriptions') || '[]');
        return newSubs.find((s: Subscription) => s.id === subscriptionId);
    }, [subscriptionId]);
